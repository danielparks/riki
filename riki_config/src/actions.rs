...
impl Action<'_> {
    ...
    /// # Errors
    ///
    /// Returns [`riki_actions::Error`] if there is a problem evaluating a
    /// function or opening a path.
...
functions! {
    ...
    Canonical(path) => {
        ...
        if *url_path == *context.variables.request_path() {
            ...
            Err(riki_actions::Error::RedirectCanonical(url_path.into_owned()))
        }
    },
    ...
    Error(code) => {
        ...
        Err(riki_actions::Error::InternalString(code.evaluate_as_string(context)?))
    },
    ...
    /// ```
    Markdown(file) => {
        riki_actions::markdown_to_html(context, file.evaluate(context)?)?
            .into()
    },
    ...
    /// ```
    RedactSource(file) => {
        riki_actions::redact_source(context, file.evaluate(context)?)?
            .into()
    },
    ...
    /// ```
    Render(file) => {
        riki_actions::render(context, file.evaluate(context)?)?
            .into()
    },
}
