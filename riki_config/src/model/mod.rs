...
use riki_actions::{self, RequestVariables, Return, VariableMap};
use riki_render::TemplatesManager;
...
impl<'src> ConfigRule<'src> {
    ...
    /// # Errors
    ///
    /// Most variants of [`riki_actions::Error`] should be returned as an HTTP
    /// response, except [`riki_actions::Error::NotFound`], which means that
    /// this rule should be skipped and the next rule evaluated.
    pub fn evaluate(
        &self,
        manager: &TemplatesManager,
        variables: RequestVariables<'_>,
    ) -> riki_actions::Result<axum::response::Response> {
        let templates_path = self.settings.templates.path_content(&variables);
        let context = riki_actions::Context {
        ...
        match self.action.evaluate(&context).and_then(
            ...
            Err(riki_actions::Error::NotFound) => {
