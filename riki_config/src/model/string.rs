...
use crate::errors::{ParseError, ParseResult, SpannedErrors};
use crate::parser2::{StringToken, StringType};
use logos::Logos;
use riki_actions::{Variable, VariableMap};
use riki_misc::bitfilter::BitFilter;
use std::borrow::Cow;
use std::ops::Range;
use std::slice;
...
impl<'src> From<&'src str> for ParsedString<'src> {
    ...
    ///
    /// ```
    /// use riki_config::model::ParsedString;
    /// use riki_actions::StaticVariables;
    /// assert_eq!(
    ///     ParsedString::from(r"\n${clean_path}")
