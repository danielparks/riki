...
use crate::{ContentReturn, MediaType, Result, Source, StaticContext};
use assert2::check;
use jiff::Timestamp;
use regex::Regex;
...
fn render(html: &str) -> Result<String> {
    static CONTEXT: LazyLock<StaticContext> = LazyLock::new(|| {
        let mut context = StaticContext::default();
        Arc::get_mut(&mut context.tpls)
            .unwrap()
            .register_template_string("default", "{{{body}}}")
            .unwrap();
        context
    });
    ...
    crate::render(&*CONTEXT, ret)?.body.into_string()
}
